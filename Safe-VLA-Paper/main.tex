...
To support complex geometries and multiple simultaneous hazards systematically, the holistic SBF is strictly compositional, defined as a minimum over constituent constraints:
\begin{equation}
h_{\text{total}}(x) = \min \left( h_{\text{human}}(x),\; h_{\text{zone}}(x),\; h_{\text{obstacle}}(x) \right)
\end{equation}
...
To combat the failing static assumptions of architectures like SaferPath, FleetSafe-VLA employs an 8D cognitive state capturing spatial, temporal, and semantic variables. Mathematically, the cognitive state $s$ fundamentally expands standard models into an 8D formulation:
\begin{equation}
...
\end{equation}
By explicitly isolating physical geometry, actuation delay bound ($\Delta t$), language goal alignment ($g$), surrounding agent intent ($\iota$), and system-wide awareness ($a$), the Semantic Barrier Functions are directly parametrised into an 8-dimensional space. This explicit composition enables delay robustness while decisively discarding unsafe static-environment assumptions.
