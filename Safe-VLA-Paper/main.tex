...
Building upon Dimensional OS (DimOS)~\cite{dimos2025}---an emerging agent-native operating system for physical space that provides seamless, ROS-free hardware abstraction---our system integrates three novel components: (1) NVIDIA GR00T N1.6 as a dual-system backbone for cross-embodiment policy learning routed via DimOS LCM messages, (2) Control Barrier Function Quadratic Programming (CBF-QP) filters that enforce forward invariance of safe sets in real time, and (3) a Deadline-Sensitive Safety Envelope Orchestrator (DSEO) providing hard real-time preemption under \SI{5}{\milli\second}.
...
\begin{itemize}
    \item A \textbf{layered safety architecture} combining GR00T N1.6 foundation model generalisation with CBF-QP provable safety executing natively on DimOS.
