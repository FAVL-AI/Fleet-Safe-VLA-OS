...
Evaluated across two platforms --- Yahboom mobile base (161.77M parameters, 2-DOF) and Unitree G1 humanoid (162.48M parameters, 23-DOF) --- alongside a coordinated fleet of 3 physical Yahbooms for lab validation, FLEET-Safe VLA achieves an 88.5\% safety cost reduction vs.\ SafeVLA (0.0007 vs.\ 0.156), 93.3\% lower inference latency (\SI{<8}{\milli\second} vs.\ \SI{120}{\milli\second}), and $14\times$ compute cost reduction (\$0.49 vs.\ \$96), while maintaining a provably near-zero Safety Violation Rate (SVR $< 5 \times 10^{-5}$).
...
Vision-Language-Action (VLA) models have rapidly advanced robotic autonomy by enabling systems to interpret high-level semantic instructions and execute complex behaviors in unstructured environments. However, despite these advances, current VLA systems fundamentally lack formal safety guarantees, particularly in dynamic, multi-agent settings where humans, robots, and unforeseen obstacles interact unpredictably.

Existing approaches, such as SaferPath, incorporate safety through constrained optimization over static traversability maps. While effective in structured or slow-moving environments, these methods implicitly assume that the world is temporally static at each decision step. This assumption breaks down under realistic conditions involving latency, occlusion, and moving agents, where safety violations arise not from instantaneous geometry, but from unmodeled future interactions.

In this work, we argue that safe deployment of VLA systems requires a shift from \emph{static spatial safety} to \emph{dynamic spatio-temporal safety}. We are the first to unify semantic reasoning, temporal prediction, and formal safety guarantees under latency in VLA systems. We introduce \textbf{FleetSafe-VLA}, a framework that enforces safety over predicted trajectories using a delay-aware control-theoretic formulation.

Our key insight is that safety must be defined over time-evolving states that jointly capture perception, prediction, and semantic intent. To this end, we propose a 7D cognitive state representation that augments classical spatial variables with temporal and semantic dimensions, including goal alignment, intent, and awareness. Building on this representation, we develop \emph{Semantic Barrier Functions (SBFs)} that encode language-conditioned constraints, and integrate them with delay-robust \emph{Control Barrier Functions (CBFs)} to guarantee forward invariance of safe sets under bounded latency and estimation error.

We evaluate FleetSafe-VLA in a high-fidelity Simulation-to-Twin (Sim-to-Twin) digital twin that captures realistic sensing, actuation, and multi-agent dynamics. By injecting controlled latency and noise, we systematically evaluate robustness. FleetSafe-VLA significantly outperforms SaferPath, reducing Safety Violation Rate (SVR) by an order of magnitude, increasing Time-To-Preempt (TTP), and maintaining low Deadline Miss Rate (DMR).

Finally, we demonstrate that inference efficiency can be improved independently of safety through KV-cache compression, achieving substantial VRAM reductions without affecting safety-critical metrics.
...
\textbf{Concern 1: Is the 7D state over-parameterized?}

\emph{Response:}
Our ablation study shows that removing individual components (temporal, semantic, or multi-agent variables) significantly degrades performance. In particular, removing temporal variables increases SVR by over 20\%, demonstrating that each dimension contributes meaningfully to safety.

\textbf{Concern 2: Does the delay-aware formulation hold in practice?}

\emph{Response:}
We explicitly evaluate performance under latency injection up to 200ms. Results show graceful degradation with bounded SVR and DMR, consistent with our theoretical guarantees. Without the delay term, SVR increases sharply, validating its necessity.
...
\emph{Response:}
Intent and awareness are estimated from trajectory history and visual attention signals. We assume bounded estimation error and explicitly incorporate this into our theoretical formulation. Empirical results show robustness under noise perturbations.
