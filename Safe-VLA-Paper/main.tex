...
First, we propose \textbf{Constrained-CMDP Policy Learning}, formulating language-vision constraint coordination in latent feature spaces. Second, we develop \textbf{Importance-Weighted Near-Violation Learning}, a VLA-specific approach to rare-event safety. Third, we establish a \textbf{Safety-Transfer Benchmark} with a systematic taxonomy of visual, physical, and semantic distribution shifts. 

Integrating these contributions, \textbf{FLEET-Safe VLA} provides strict mathematical safety guarantees. Evaluated on a 2-DOF Yahboom mobile base, a 23-DOF Unitree G1, and a coordinated physical fleet, preliminary simulation results suggest a potential for an 80--84\% reduction in Safety Violation Rate (SVR) with acceptable latency ($\le$\SI{20}{\milli\second}) on embedded hardware, vastly outperforming SafeVLA baselines.
\end{abstract}
...
This research systematically addresses three specific gaps that SafeVLA and related work leave open. We introduce \textbf{FleetSafe-VLA}, a framework that enforces safety over predicted trajectories using a delay-aware control-theoretic formulation.

\textbf{Contributions.}
\begin{itemize}
    \item \textbf{Constrained-CMDP Policy Learning}: We extend Constrained Markov Decision Processes to formally model language-vision constraint coordination in high-dimensional feature spaces. Unlike SafeVLA, which applies standard CMDP solvers directly to VLA observations, we formulate the problem in latent feature space where language instructions explicitly modulate safety constraints on visual features.
    \item \textbf{Importance-Weighted Near-Violation Learning}: A VLA-specific approach to rare-event safety learning. While existing methods like SIM2VAL provide general cross-platform variance reduction, our contribution is tailored to multi-modal vision-language feature distributions, evaluated on non-IID real-world data distributions (e.g., Oxford RobotCar).
    \item \textbf{Safety-Transfer Benchmark}: A systematic taxonomy of visual, physical, and semantic distribution shifts. While Safety-CHORES provides in-distribution benchmarking, this work characterises when and how learned safety properties transfer to OOD environments.
    \item Formal safety guarantees under bounded latency and estimation uncertainty using \emph{Semantic Barrier Functions (SBFs)} and a 7D cognitive state representation.
\end{itemize}
...
\subsection{Importance-Weighted Near-Violation Learning}
To consistently handle rare safety-relevant events, we develop a dedicated learning module. While existing variance reduction techniques like SIM2VAL focus on cross-platform dynamics, our approach is tailored to the heavy-tailed nature of vision-language distributions. By over-sampling near-violation states proportional to their importance weights, the policy learns robust recovery behaviours, validated on non-IID real-world environments analogous to the Oxford RobotCar dataset.
...
Pairing a large foundation model backbone with a CBF-QP filter and hardware-level preemption yielded an 84\% drop in safety violations and acceptable real-time latency ($\le$\SI{20}{\milli\second}) over SafeVLA. Coupled with Importance-Weighted Near-Violation Learning and the newly established Safety-Transfer Benchmark, the framework addresses severe gaps in current VLA deployment architectures.
