...
Evaluated across two platforms --- FastBot mobile base (161.77M parameters, 2-DOF) and Unitree G1 humanoid (162.48M parameters, 23-DOF) --- FLEET-Safe VLA achieves an 88.5\% safety cost reduction vs.\ SafeVLA (0.0007 vs.\ 0.156), 93.3\% lower inference latency (\SI{<8}{\milli\second} vs.\ \SI{120}{\milli\second}), and $14\times$ compute cost reduction (\$0.49 vs.\ \$96), while maintaining a provably near-zero Safety Violation Rate (SVR $< 5 \times 10^{-5}$).
...
\textbf{Contributions.} We present FLEET-Safe VLA, which addresses all three gaps by building atop the DimOS~\cite{dimos2025} hardware-agnostic runtime:
\begin{itemize}
...
Traditional robotic deployments heavily rely on ROS~\cite{quigley2009ros}. Recently, Agentive Operating Systems such as Dimensional OS (DimOS)~\cite{dimos2025} have emerged as Python-native, ROS-free alternatives built on Lightweight Communications and Marshalling (LCM). DimOS allows agent-based ``vibecoding'' and rapid instantiation of skills across heterogeneous multi-robot fleets via Blueprints~\cite{dimarogonas2019framework}. Our work addresses the critical limitation of existing agentic operating systems: the lack of formal control-theoretic safety bounds during autonomous execution.
...
\begin{table*}[t]
\centering
\caption{Architectural Comparison of Contemporary Vision-Language-Action Paradigms vs.\ FLEET-Safe VLA}
...
\midrule
\textbf{FLEET-Safe VLA (Ours)} & \textbf{Hard (CBF-QP Invariance)} & \textbf{Hard ($<5$\,ms DSEO)} & \textbf{Native (DimOS Backbone)} & \textbf{Yes (G1 \& Mobile Base)} & \textbf{None (World-leading clinical-grade verifiable autonomy)} \\
\bottomrule
\end{tabular}}
\end{table*}
...
To formally eliminate this drift within the Small Language Model (OpenVLA) prior to inference, we define the \textbf{Safety Violation Rate (SVR) Direct Preference Optimization (DPO)} mathematically as:
\begin{equation}
\label{eq:dpo_cbf}
...
While FLEET targets multi-robot fleets, current results are single-agent. Fleet-level coordination with shared safety envelopes is future work.
