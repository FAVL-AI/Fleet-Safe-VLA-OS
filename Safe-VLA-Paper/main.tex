...
By integrating these contributions, \textbf{FleetSafe-VLA} establishes strict formal safety guarantees targeting dynamic, unmodelled future interactions where static methods like SaferPath fail. By using the cost-effective 2-DOF Yahboom mobile base as our primary hardware testbed for \textbf{Sim-to-Twin} physical validation, we demonstrate that these formulations democratise safe VLA deployment. Evaluated across physical Yahboom fleets and cross-embodied on a 23-DOF Unitree G1, preliminary simulation results show a potential for an 80--84\% reduction in Safety Violation Rate (SVR) with acceptable latency ($\le$\SI{20}{\milli\second}) on embedded hardware, outperforming standard SafeVLA baselines.
\end{abstract}
