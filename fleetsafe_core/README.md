...
<table>
  ...
  <tr>
    <td align="center" width="50%">
      ...
      SLAM, dynamic obstacle avoidance, route planning, and autonomous exploration — via both FleetSafeCore native and ROS<br><a href="https://x.com/stash_pomichter/status/2010471593806545367">Watch video</a>
    </td>
    <td align="center" width="50%">
      <h3>Perception</h3>
...
<table>
  ...
  <tr>
    <td align="center" width="20%">
      ...
      🟥 <a href="fleetsafe_core/robot/unitree/b1">Unitree B1</a><br>
    </td>
    <td align="center" width="20%">
    ...
    <td align="center" width="20%">
      🟧 <a href="fleetsafe_core/robot/drone/README.md">MAVLink</a><br>
...
uv pip install 'fleetsafe_core[base,unitree]'
...
fleetsafe_core --replay run unitree-go2
```

```bash
# Install with simulation support
uv pip install 'fleetsafe_core[base,unitree,sim]'

# Run quadruped in MuJoCo simulation
fleetsafe_core --simulation run unitree-go2

# Run humanoid in simulation
fleetsafe_core --simulation run unitree-g1-sim
```

```bash
# Control a real robot (Unitree quadruped over WebRTC)
export ROBOT_IP=<YOUR_ROBOT_IP>
fleetsafe_core run unitree-go2
```
...
|-------------|-------------|
| `fleetsafe_core --replay run unitree-go2` | Quadruped navigation replay — SLAM, costmap, A* planning |
| `fleetsafe_core --replay --replay-dir unitree_go2_office_walk2 run unitree-go2-temporal-memory` | Quadruped temporal memory replay |
| `fleetsafe_core --simulation run unitree-go2-agentic-mcp` | Quadruped agentic + MCP server in simulation |
| `fleetsafe_core --simulation run unitree-g1` | Humanoid in MuJoCo simulation |
| `fleetsafe_core --replay run drone-basic` | Drone video + telemetry replay |
| `fleetsafe_core --replay run drone-agentic` | Drone + LLM agent with flight skills (replay) |
| `fleetsafe_core run demo-camera` | Webcam demo — no hardware needed |
| `fleetsafe_core run keyboard-teleop-xarm7` | Keyboard teleop with mock xArm7 (requires `fleetsafe_core[manipulation]` extra) |
| `fleetsafe_core --simulation run unitree-go2-agentic-ollama` | Quadruped agentic with local LLM (requires [Ollama](https://ollama.com) + `ollama serve`) |

> Full blueprint docs: [docs/usage/blueprints.md](docs/usage/blueprints.md)

# Agent CLI and MCP

The `fleetsafe_core` CLI manages the full lifecycle — run blueprints, inspect state, interact with agents, and call skills via MCP.

```bash
fleetsafe_core run unitree-go2-agentic-mcp --daemon   # Start in background
fleetsafe_core status                              # Check what's running
fleetsafe_core log -f                              # Follow logs
fleetsafe_core agent-send "explore the room"       # Send agent a command
fleetsafe_core mcp list-tools                      # List available MCP skills
fleetsafe_core mcp call relative_move --arg forward=0.5  # Call a skill directly
fleetsafe_core stop                                # Shut down
```
...
## Use FleetSafeCore as a Library

See below a simple robot connection module that sends streams of continuous `cmd_vel` to the robot and receives `color_image` to a simple `Listener` module. FleetSafeCore Modules are subsystems on a robot that communicate with other modules using standardized messages.
...
from fleetsafe_core.msgs.geometry_msgs import Twist
from fleetsafe_core.msgs.sensor_msgs import Image, ImageFormat
...
from fleetsafe_core.robot.unitree.go2.connection import go2_connection
from fleetsafe_core.agents.agent import agent
...
<img src="assets/readme/fleetsafe_core_demo.gif" alt="FleetSafeCore Demo" width="100%">

# Development

## Develop on FleetSafeCore
