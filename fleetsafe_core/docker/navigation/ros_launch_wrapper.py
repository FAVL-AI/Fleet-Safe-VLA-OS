...
class ROSLaunchWrapper:
    def __init__(self):
        ...
        self.fleetsafe_core_process = None
    ...
    def signal_handler(self, _signum, _frame):
        ...
        # Stop FleetSafeCore first
        if self.fleetsafe_core_process and self.fleetsafe_core_process.poll() is None:
            print("Stopping FleetSafeCore...")
            self.fleetsafe_core_process.terminate()
            try:
                self.fleetsafe_core_process.wait(timeout=5)
                print("FleetSafeCore stopped cleanly.")
            except subprocess.TimeoutExpired:
                print("Force stopping FleetSafeCore...")
                self.fleetsafe_core_process.kill()
                self.fleetsafe_core_process.wait()
    ...
    def run(self):
        ...
        print("Starting ROS route planner and FleetSafeCore...")
        ...
        print("Starting FleetSafeCore navigation bot...")

        nav_bot_path = "/workspace/fleetsafe_core/fleetsafe_core/navigation/demo_ros_navigation.py"
        venv_python = "/opt/fleetsafe_core-venv/bin/python"

        if not os.path.exists(nav_bot_path):
            print(f"ERROR: demo_ros_navigation.py not found at {nav_bot_path}")
            nav_dir = "/workspace/fleetsafe_core/fleetsafe_core/navigation/"
        ...
        try:
            self.fleetsafe_core_process = subprocess.Popen(
            ...
            time.sleep(2)
            poll_result = self.fleetsafe_core_process.poll()
            if poll_result is not None:
                # Process exited immediately
                stdout, stderr = self.fleetsafe_core_process.communicate(timeout=1)
                print(f"ERROR: FleetSafeCore failed to start (exit code: {poll_result})")
                if stdout:
                ...
                print(f"FleetSafeCore started successfully (PID: {self.fleetsafe_core_process.pid})")

        except Exception as e:
            print(f"ERROR: Failed to start FleetSafeCore: {e}")
            self.fleetsafe_core_process = None

        if self.fleetsafe_core_process:
            print("Both systems are running. Press Ctrl+C to stop.")
        else:
            print("ROS is running (FleetSafeCore failed to start). Press Ctrl+C to stop.")
        print("")
        ...
        try:
            ...
            while True:
                ...
                if self.ros_process.poll() is not None:
                    ...
                    break
                if self.fleetsafe_core_process and self.fleetsafe_core_process.poll() is not None:
                    print("FleetSafeCore process has stopped.")
                    # FleetSafeCore stopping is less critical, but we should still clean up ROS
                    self.signal_handler(signal.SIGTERM, None)
                    break
                time.sleep(1)
