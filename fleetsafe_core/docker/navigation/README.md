# ROS Docker Integration for FleetSafeCore

This directory contains Docker configuration files to run FleetSafeCore and the ROS autonomy stack in the same container, enabling communication between the two systems.
...
- Set up the environment for both ROS and FleetSafeCore

The resulting image will be named `fleetsafe_core_autonomy_stack:{distro}` (e.g., `humble`, `jazzy`).
...
```bash
cd ../..  # Back to fleetsafe_core root
...
2. Load the layout from `fleetsafe_core/assets/foxglove_dashboards/Overwatch.json` (Layout menu → Import)
...
```bash
docker exec -it fleetsafe_core_hardware_container bash
```
...
For the Unitree G1
```bash
fleetsafe_core run unitree-g1
ROBOT_IP=XX.X.X.XXX fleetsafe_core run unitree-g1 # If ROBOT_IP env variable is not set in .env
```
