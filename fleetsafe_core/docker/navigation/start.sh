...
echo -e "${GREEN}================================================${NC}"
echo -e "${GREEN}Starting FleetSafeCore Docker Container${NC}"
...
if ! docker images --format '{{.Repository}}:{{.Tag}}' | grep -q "^fleetsafe_core_autonomy_stack:${IMAGE_TAG}$"; then
    echo -e "${RED}Docker image fleetsafe_core_autonomy_stack:${IMAGE_TAG} not found.${NC}"
...
if [ "$MODE" = "hardware" ]; then
    CONTAINER_NAME="fleetsafe_core_hardware_container"
elif [ "$MODE" = "bagfile" ]; then
    CONTAINER_NAME="fleetsafe_core_bagfile_container"
else
    CONTAINER_NAME="fleetsafe_core_simulation_container"
fi
...
if [ "$MODE" = "hardware" ]; then
    ...
    echo "Simulation mode - Auto-starting ROS simulation and FleetSafeCore"
    echo ""
    echo "The container will automatically run:"
    echo "  - ROS navigation stack with route planner"
    echo "  - FleetSafeCore navigation demo"
    echo ""
    echo "To enter the container from another terminal:"
