...
from fleetsafe_core.hardware.sensors.camera.spec import (
    OPTICAL_ROTATION,
    DepthCameraConfig,
    DepthCameraHardware,
)
from fleetsafe_core.msgs.geometry_msgs import Quaternion, Transform, Vector3
from fleetsafe_core.msgs.sensor_msgs import CameraInfo
...
from fleetsafe_core.robot.foxglove_bridge import FoxgloveBridge
from fleetsafe_core.spec import perception
...
def main() -> None:
    fleetsafe_core = ModuleCoordinator(n=2)
    fleetsafe_core.start()

    camera = fleetsafe_core.deploy(RealSenseCamera, enable_pointcloud=True, pointcloud_fps=5.0)  # type: ignore[type-var]
    ...
    fleetsafe_core.start_all_modules()
