...
Usage:
    ...
    fleetsafe_core run coordinator-mock          # Single arm
    fleetsafe_core run coordinator-dual-mock     # Dual arm

    # Terminal 2: Run this client
    python -m fleetsafe_core.manipulation.control.coordinator_client
    python -m fleetsafe_core.manipulation.control.coordinator_client --task traj_left
    python -m fleetsafe_core.manipulation.control.coordinator_client --task traj_right
...
from fleetsafe_core.manipulation.planning.trajectory_generator.joint_trajectory_generator import (
...
def _run_client(client: CoordinatorClient, task: str, vel: float, accel: float) -> int:
    ...
    try:
        ...
        if not hardware:
            ...
            print("Start with: fleetsafe_core run coordinator-mock")
        ...
        print(f"\nConnection error: {e}")
        print("Make sure coordinator is running: fleetsafe_core run coordinator-mock")
        return 1
