...
from fleetsafe_core.protocol.pubsub.impl.lcmpubsub import Topic
from fleetsafe_core.protocol.service.lcmservice import (
    _DEFAULT_LCM_URL,
    LCMConfig,
    LCMService,
    autoconf,
)
from fleetsafe_core.protocol.service.system_configurator import (
...
class TestConfigureSystemForLcm:
    def test_creates_linux_checks_on_linux(self) -> None:
        with patch(
            "fleetsafe_core.protocol.service.system_configurator.platform.system", return_value="Linux"
        ):
            with patch("fleetsafe_core.protocol.service.lcmservice.configure_system") as mock_configure:
                autoconf()
    ...
    def test_creates_macos_checks_on_darwin(self) -> None:
        with patch(
            "fleetsafe_core.protocol.service.system_configurator.platform.system", return_value="Darwin"
        ):
            with patch("fleetsafe_core.protocol.service.lcmservice.configure_system") as mock_configure:
                autoconf()
                mock_configure.assert_called_once()
    ...
    def test_logs_error_on_unsupported_system(self) -> None:
        with patch(
            "fleetsafe_core.protocol.service.system_configurator.platform.system", return_value="Windows"
        ):
            with patch("fleetsafe_core.protocol.service.lcmservice.configure_system") as mock_configure:
                with patch("fleetsafe_core.protocol.service.lcmservice.logger") as mock_logger:
                    autoconf()
...
class TestLCMService:
    def test_init_with_default_config(self) -> None:
        with patch("fleetsafe_core.protocol.service.lcmservice.lcm.LCM") as mock_lcm_class:
