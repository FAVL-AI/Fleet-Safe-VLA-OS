...
from fleetsafe_core.msgs.sensor_msgs.Image import Image, ImageFormat
from fleetsafe_core.protocol.pubsub.benchmark.type import Case
...
try:
    ...
    DDS_AVAILABLE = False
from fleetsafe_core.protocol.pubsub.impl.lcmpubsub import LCM, LCMPubSubBase, Topic as LCMTopic
from fleetsafe_core.protocol.pubsub.impl.memory import Memory
from fleetsafe_core.protocol.pubsub.impl.shmpubsub import (
    BytesSharedMemory,
    LCMSharedMemory,
    PickleSharedMemory,
)
...
if DDS_AVAILABLE:
    ...
    from fleetsafe_core.protocol.pubsub.impl.ddspubsub import (
        DDS,
        Topic as DDSTopic,
    )
...
try:
    from fleetsafe_core.protocol.pubsub.impl.redispubsub import Redis
...
from fleetsafe_core.protocol.pubsub.impl.rospubsub import (
    ROS_AVAILABLE,
    FleetSafeCoreROS,
    RawROS,
    RawROSTopic,
...
if ROS_AVAILABLE:
    ...
    @contextmanager
    def fleetsafe_core_ros_best_effort_pubsub_channel() -> Generator[FleetSafeCoreROS, None, None]:
        qos = QoSProfile(  # type: ignore[no-untyped-call]
            reliability=QoSReliabilityPolicy.BEST_EFFORT,
            history=QoSHistoryPolicy.KEEP_LAST,
            durability=QoSDurabilityPolicy.VOLATILE,
        ...
        ros_pubsub = FleetSafeCoreROS(node_name="benchmark_fleetsafe_core_ros_best_effort", qos=qos)
        ros_pubsub.start()
        yield ros_pubsub
        ros_pubsub.stop()

    @contextmanager
    def fleetsafe_core_ros_reliable_pubsub_channel() -> Generator[FleetSafeCoreROS, None, None]:
        qos = QoSProfile(  # type: ignore[no-untyped-call]
            reliability=QoSReliabilityPolicy.RELIABLE,
            history=QoSHistoryPolicy.KEEP_LAST,
            durability=QoSDurabilityPolicy.VOLATILE,
        ...
        ros_pubsub = FleetSafeCoreROS(node_name="benchmark_fleetsafe_core_ros_reliable", qos=qos)
        ros_pubsub.start()
        yield ros_pubsub
        ros_pubsub.stop()

    def fleetsafe_core_ros_msggen(size: int) -> tuple[ROSTopic, Image]:
        topic = ROSTopic(topic="/benchmark/fleetsafe_core_ros", msg_type=Image)
        return (topic, make_data_image(size))
    ...
    # testcases.append(
    #     Case(
    #         pubsub_context=fleetsafe_core_ros_best_effort_pubsub_channel,
    #         msg_gen=fleetsafe_core_ros_msggen,
    #     )
    # )

    testcases.append(
        Case(
            pubsub_context=fleetsafe_core_ros_reliable_pubsub_channel,
            msg_gen=fleetsafe_core_ros_msggen,
        )
    )
