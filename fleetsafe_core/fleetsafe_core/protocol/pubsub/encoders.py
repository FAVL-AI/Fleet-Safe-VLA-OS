...
class LCMTopicProto(Protocol):
    ...
    lcm_type: type[FleetSafeCoreMsg] | None


class LCMEncoderMixin(PubSubEncoderMixin[LCMTopicProto, FleetSafeCoreMsg, bytes]):
    """Encoder mixin for FleetSafeCoreMsg using LCM binary encoding."""

    def encode(self, msg: FleetSafeCoreMsg | bytes, _: LCMTopicProto) -> bytes:
        if isinstance(msg, bytes):
            return msg
        return msg.lcm_encode()

    def decode(self, msg: bytes, topic: LCMTopicProto) -> FleetSafeCoreMsg:
...
class JpegEncoderMixin(PubSubEncoderMixin[LCMTopicProto, Image, bytes]):
    """Encoder mixin for FleetSafeCoreMsg using JPEG encoding (for images)."""
