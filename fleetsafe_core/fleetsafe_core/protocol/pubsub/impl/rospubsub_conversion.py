...
"""Conversion functions between fleetsafe_core messages and ROS messages.

This module provides conversion functions between fleetsafe_core message types and ROS messages.
...
if TYPE_CHECKING:
    from fleetsafe_core.msgs import FleetSafeCoreMsg
    from fleetsafe_core.protocol.pubsub.impl.rospubsub import ROSMessage


# Complex types that need LCM roundtrip (explicit list)
# These types have different internal representations in fleetsafe_core vs ROS/LCM
...
# Cache for dynamic imports of fleetsafe_core types
_fleetsafe_core_type_cache: dict[str, type[FleetSafeCoreMsg] | None] = {}
...
def get_fleetsafe_core_type(msg_name: str) -> type[FleetSafeCoreMsg] | None:
    """Try to import fleetsafe_core.msgs type, return None if not found. Cached.

    Args:
        msg_name: Message name in format "package.MessageName" (e.g., "geometry_msgs.Vector3")

    Returns:
        The fleetsafe_core message type, or None if not found
    """
    if msg_name in _fleetsafe_core_type_cache:
        return _fleetsafe_core_type_cache[msg_name]

    try:
        package, name = msg_name.split(".")
        module = importlib.import_module(f"fleetsafe_core.msgs.{package}.{name}")
        fleetsafe_core_type = cast("type[FleetSafeCoreMsg]", getattr(module, name))
        _fleetsafe_core_type_cache[msg_name] = fleetsafe_core_type
        return fleetsafe_core_type
    except (ImportError, AttributeError, ValueError):
        _fleetsafe_core_type_cache[msg_name] = None
        return None


def derive_lcm_type(fleetsafe_core_type: type[FleetSafeCoreMsg]) -> type[Any]:
    """Derive the LCM message type from a fleetsafe_core message type.
...
def derive_ros_type(fleetsafe_core_type: type[FleetSafeCoreMsg]) -> type[ROSMessage]:
    """Derive the ROS message type from a fleetsafe_core message type.

    Args:
        fleetsafe_core_type: A fleetsafe_core message type (e.g., fleetsafe_core.msgs.geometry_msgs.Vector3)
    ...
    """
    msg_name = fleetsafe_core_type.msg_name  # e.g., "geometry_msgs.Vector3"
    parts = msg_name.split(".")
    if len(parts) != 2:
        raise ValueError(f"Invalid msg_name format: {msg_name}, expected 'package.MessageName'")
...
def fleetsafe_core_to_ros(msg: FleetSafeCoreMsg, ros_type: type[ROSMessage]) -> ROSMessage:
    """Convert a fleetsafe_core message to a ROS message.
    ...
    Args:
        msg: FleetSafeCore message instance
    ...
    if msg_name in COMPLEX_TYPES:
        # Complex: fleetsafe_core → encode → decode LCM → copy to ROS
...
def ros_to_fleetsafe_core(msg: Any, fleetsafe_core_type: type[FleetSafeCoreMsg]) -> FleetSafeCoreMsg:
    """Convert a ROS message to a fleetsafe_core message.

    For complex types (PointCloud2, Image, CameraInfo), uses LCM roundtrip
    to properly build the fleetsafe_core internal representation. For simple types,
    uses direct field copy.

    Args:
        msg: ROS message instance
        fleetsafe_core_type: Target fleetsafe_core message type

    Returns:
        FleetSafeCore message instance
    """
    msg_name = fleetsafe_core_type.msg_name

    if msg_name in COMPLEX_TYPES:
        # Complex: ROS → LCM → encode → decode → fleetsafe_core
        lcm_type = derive_lcm_type(fleetsafe_core_type)
        lcm_msg = lcm_type()
        _copy_ros_to_lcm_recursive(msg, lcm_msg)
        return fleetsafe_core_type.lcm_decode(lcm_msg.lcm_encode())

    # Simple type: recursive field copy (handles nested messages)
    fleetsafe_core_msg = fleetsafe_core_type()
    _copy_ros_to_lcm_recursive(msg, fleetsafe_core_msg)
    return fleetsafe_core_msg
