...
from fleetsafe_core.msgs.sensor_msgs.PointCloud2 import PointCloud2
from fleetsafe_core.protocol.pubsub.impl.rospubsub import FleetSafeCoreROS, ROSTopic
...
from fleetsafe_core.utils.testing import TimedSensorReplay
...
@pytest.fixture()
def publisher() -> Generator[FleetSafeCoreROS, None, None]:
    yield from ros_node()


@pytest.fixture()
def subscriber() -> Generator[FleetSafeCoreROS, None, None]:
    yield from ros_node()
...
def test_basic_conversion(publisher, subscriber):
    ...
    Simple flat fleetsafe_core.msgs type with no nesting (just x/y/z floats).
    """
...
def test_posestamped_pubsub(publisher, subscriber):
    ...
    COMPLEX_TYPE with custom fleetsafe_core.msgs implementation and nested messages
...
def test_pointstamped_pubsub(publisher, subscriber):
    ...
    Raw LCM type with nested messages (Header, Point) but NO custom fleetsafe_core.msgs
...
def test_twist_pubsub(publisher, subscriber):
    ...
    fleetsafe_core.msgs type with nested Vector3 messages (linear, angular).
    Tests recursive field copy with custom fleetsafe_core.msgs nested types.
    """
    original = Twist(
        linear=[1.0, 2.0, 3.0],
        angular=[0.1, 0.2, 0.3],
