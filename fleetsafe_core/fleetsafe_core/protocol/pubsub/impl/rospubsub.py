...
from fleetsafe_core.protocol.pubsub.impl.rospubsub_conversion import (
    derive_ros_type,
    fleetsafe_core_to_ros,
    ros_to_fleetsafe_core,
)
...
@dataclass
class ROSTopic:
    """Topic descriptor for FleetSafeCoreROS pubsub (uses fleetsafe_core message types)."""

    topic: str
    msg_type: type[FleetSafeCoreMsg]
    qos: "QoSProfile | None" = None
...
class RawROS(PubSub[RawROSTopic, Any]):
    ...
    def __init__(self, node_name: str | None = None, qos: "QoSProfile | None" = None) -> None:
        ...
        self._node_name = node_name or f"fleetsafe_core_ros_{uuid.uuid4().hex[:8]}"
        ...
        if qos is not None:
            ...
            self._qos = QoSProfile(  # type: ignore[no-untyped-call]
                ...
                # ./bin/dev python -m pytest -svm tool -k ros fleetsafe_core/protocol/pubsub/benchmark/test_benchmark.py
                #
...
class FleetSafeCoreROS(PubSub[ROSTopic, FleetSafeCoreMsg]):
    """ROS PubSub with automatic fleetsafe_core.msgs ↔ ROS message conversion.

    Uses ROSTopic (with fleetsafe_core msg_type) instead of RawROSTopic (with ros_type).
    Automatically converts between fleetsafe_core and ROS message formats.
    Uses composition with RawROS internally.
    """

    def __init__(self, node_name: str | None = None, qos: "QoSProfile | None" = None) -> None:
        """Initialize the FleetSafeCoreROS pubsub.
    ...
    def publish(self, topic: ROSTopic, message: FleetSafeCoreMsg) -> None:
        """Publish a fleetsafe_core message to a ROS topic.

        Args:
            topic: ROSTopic with fleetsafe_core msg_type
            message: FleetSafeCore message to publish
        """
        raw_topic = self._to_raw_topic(topic)
        ros_message = fleetsafe_core_to_ros(message, raw_topic.ros_type)
        self._raw.publish(raw_topic, ros_message)

    def subscribe(
        self, topic: ROSTopic, callback: Callable[[FleetSafeCoreMsg, ROSTopic], None]
    ) -> Callable[[], None]:
        """Subscribe to a ROS topic with automatic fleetsafe_core message conversion.

        Args:
            topic: ROSTopic with fleetsafe_core msg_type
            callback: Function called with (fleetsafe_core_message, topic)
        ...
        def wrapped_callback(ros_msg: Any, _raw_topic: RawROSTopic) -> None:
            fleetsafe_core_msg = ros_to_fleetsafe_core(ros_msg, topic.msg_type)
            callback(fleetsafe_core_msg, topic)

        return self._raw.subscribe(raw_topic, wrapped_callback)


ROS = FleetSafeCoreROS
