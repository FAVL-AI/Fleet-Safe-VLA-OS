...
_UNLINK_ON_GC = os.getenv("FLEETSAFE_CORE_IPC_UNLINK_ON_GC", "0").lower() not in ("0", "false", "no")
...
def _open_shm_with_retry(name: str) -> SharedMemory:
    tries = int(os.getenv("FLEETSAFE_CORE_IPC_ATTACH_RETRIES", "40"))  # ~40 tries
    base_ms = float(os.getenv("FLEETSAFE_CORE_IPC_ATTACH_BACKOFF_MS", "5"))  # 5 ms
    cap_ms = float(os.getenv("FLEETSAFE_CORE_IPC_ATTACH_BACKOFF_CAP_MS", "200"))  # 200 ms
    last = None
