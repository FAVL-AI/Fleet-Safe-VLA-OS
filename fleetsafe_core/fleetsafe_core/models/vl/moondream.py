...
from fleetsafe_core.models.base import HuggingFaceModel, HuggingFaceModelConfig
...
class MoondreamVlModel(HuggingFaceModel, VlModel):
    ...
    def _to_pil(self, image: Image | np.ndarray[Any, Any]) -> PILImage.Image:
        """Convert fleetsafe_core Image or numpy array to PIL Image, applying auto_resize."""
        if isinstance(image, np.ndarray):
            warnings.warn(
                "MoondreamVlModel should receive standard fleetsafe_core Image type, not a numpy array",
                DeprecationWarning,
                stacklevel=2,
            )
