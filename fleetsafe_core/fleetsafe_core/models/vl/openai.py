...
from fleetsafe_core.models.vl.base import VlModel, VlModelConfig
...
class OpenAIVlModel(VlModel):
    ...
    def query(self, image: Image | np.ndarray, query: str, response_format: dict | None = None, **kwargs) -> str:  # type: ignore[override, type-arg, no-untyped-def]
        if isinstance(image, np.ndarray):
            ...
            warnings.warn(
                "OpenAIVlModel.query should receive standard fleetsafe_core Image type, not a numpy array",
                DeprecationWarning,
                stacklevel=2,
            )
