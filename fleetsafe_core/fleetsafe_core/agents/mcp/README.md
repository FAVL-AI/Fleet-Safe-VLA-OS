# FleetSafeCore MCP Server

Expose FleetSafeCore robot skills to Claude Code via Model Context Protocol.
...
```bash
claude mcp add --transport http --scope project fleetsafe_core http://localhost:9990/mcp
```
...
**Terminal 1** - Start FleetSafeCore:
```bash
uv run fleetsafe_core run unitree-go2-agentic-mcp
```
