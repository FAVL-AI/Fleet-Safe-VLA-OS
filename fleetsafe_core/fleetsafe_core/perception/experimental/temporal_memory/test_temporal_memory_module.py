...
from fleetsafe_core.core.stream import Out
from fleetsafe_core.core.transport import LCMTransport
from fleetsafe_core.msgs.sensor_msgs import Image
from fleetsafe_core.perception.experimental.temporal_memory import (
    Frame,
...
from fleetsafe_core.perception.experimental.temporal_memory.entity_graph_db import EntityGraphDB
from fleetsafe_core.perception.experimental.temporal_memory.temporal_utils.graph_utils import (
    extract_time_window,
)
...
class TestPersistence:
    def test_new_memory_clears_db(self, tmp_path: Path) -> None:
        ...
        with patch(
            "fleetsafe_core.perception.experimental.temporal_memory.temporal_memory.get_run_log_dir",
            return_value=None,
        ):
            tm = TemporalMemory(
                vlm=MagicMock(),
...
class TestEntityMarkers:
    ...
    def test_markers_to_rerun(self) -> None:
        from fleetsafe_core.msgs.visualization_msgs.EntityMarkers import EntityMarkers, Marker
...
class TestWindowAnalyzer:
    def test_analyze_window_calls_vlm(self) -> None:
        from fleetsafe_core.perception.experimental.temporal_memory.window_analyzer import WindowAnalyzer
...
class TestTemporalMemoryIntegration:
    ...
    def fleetsafe_core_cluster(self):
        fleetsafe_core = ModuleCoordinator()
        fleetsafe_core.start()
        try:
            yield fleetsafe_core
        finally:
            fleetsafe_core.stop()

    @pytest.fixture(scope="function")
    def video_module(self, fleetsafe_core_cluster):
        video_module = fleetsafe_core_cluster.deploy(VideoReplayModule, num_frames=8)
    ...
    @pytest.fixture(scope="function")
    def temporal_memory_module(self, fleetsafe_core_cluster, tmp_path):
        from fleetsafe_core.models.vl.openai import OpenAIVlModel
        ...
        with patch(
            ...
            tm = fleetsafe_core_cluster.deploy(
                TemporalMemory,
                vlm=vlm,
    ...
    def test_frames_flow_and_query(
        self, fleetsafe_core_cluster, video_module, temporal_memory_module, tmp_path
    ):
