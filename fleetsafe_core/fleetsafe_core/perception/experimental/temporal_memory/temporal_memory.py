...
from fleetsafe_core.utils.logging_config import get_run_log_dir, setup_logger
...
class TemporalMemoryConfig(ModuleConfig):
    ...
    db_dir: str | Path | None = (
        None  # Persistent memory dir (default: ~/.local/state/fleetsafe_core/temporal_memory/)
    )
...
class TemporalMemory(Module):
    ...
    def __init__(
        ...
        if run_log_dir is None:
            env_dir = os.environ.get("FLEETSAFE_CORE_RUN_LOG_DIR")
            if env_dir:
