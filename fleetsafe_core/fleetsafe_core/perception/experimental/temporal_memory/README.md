...
fleetsafe_core --simulation run unitree-go2-temporal-memory

# With replay data
fleetsafe_core --replay run unitree-go2-temporal-memory
...
from fleetsafe_core.perception.experimental.temporal_memory import temporal_memory
...
```python
from fleetsafe_core.perception.experimental.temporal_memory import TemporalMemory, TemporalMemoryConfig
...
config = TemporalMemoryConfig(
    ...
    # Storage
    db_dir=None,                # Persistent DB dir (default: ~/.local/state/fleetsafe_core/temporal_memory/)
...
| JSONL dump | `~/.local/state/fleetsafe_core/temporal_memory/temporal_memory.jsonl` | Persistent | Accumulated raw VLM output across all runs |
| SQLite DB | `~/.local/state/fleetsafe_core/temporal_memory/entity_graph.db` | Persistent | Entities, relations, distances |
...
DISPLAY=:99 python -m pytest fleetsafe_core/perception/experimental/temporal_memory/test_temporal_memory_module.py -v -c /dev/null

# Integration test with real VLM
export OPENAI_API_KEY=...
DISPLAY=:99 python -m pytest fleetsafe_core/perception/experimental/temporal_memory/test_temporal_memory_module.py -v -c /dev/null -k "integration" --slow
```
