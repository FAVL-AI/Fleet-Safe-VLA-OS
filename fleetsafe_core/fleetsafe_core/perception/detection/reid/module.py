...
from fleetsafe_core.perception.detection.reid.type import IDSystem
from fleetsafe_core.perception.detection.type.detection2d.imageDetections2D import ImageDetections2D
from fleetsafe_core.types.timestamped import align_timestamped, to_ros_stamp
...
class ReidModule(Module):
    ...
    def __init__(self, idsystem: IDSystem | None = None, **kwargs) -> None:  # type: ignore[no-untyped-def]
        ...
        if idsystem is None:
            try:
                from fleetsafe_core.models.embedding import TorchReIDModel

                idsystem = EmbeddingIDSystem(model=TorchReIDModel, padding=0)  # type: ignore[arg-type]
            except Exception as e:
                raise RuntimeError(
                    "TorchReIDModel not available. Please install with: pip install fleetsafe_core[torchreid]"
                ) from e
