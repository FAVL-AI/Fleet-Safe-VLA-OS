...
from fleetsafe_core.constants import FLEETSAFE_CORE_LOG_DIR, FLEETSAFE_CORE_PROJECT_ROOT
...
def set_run_log_dir(log_dir: str | Path) -> None:
    ...
    os.environ["FLEETSAFE_CORE_RUN_LOG_DIR"] = str(log_dir)
...
def _get_log_directory() -> Path:
    ...
    if (FLEETSAFE_CORE_PROJECT_ROOT / ".git").exists():
        log_dir = FLEETSAFE_CORE_LOG_DIR
    else:
        # Running from an installed package - use XDG_STATE_HOME
        xdg_state_home = os.getenv("XDG_STATE_HOME")
        if xdg_state_home:
            log_dir = Path(xdg_state_home) / "fleetsafe_core" / "logs"
        else:
            log_dir = Path.home() / ".local" / "state" / "fleetsafe_core" / "logs"

    try:
        log_dir.mkdir(parents=True, exist_ok=True)
    except (PermissionError, OSError):
        log_dir = Path(tempfile.gettempdir()) / "fleetsafe_core" / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)
...
def _get_log_file_path() -> Path:
    ...
    env_log_dir = os.environ.get("FLEETSAFE_CORE_RUN_LOG_DIR")
    if env_log_dir:
        p = Path(env_log_dir)
    ...
    pid = os.getpid()
    return log_dir / f"fleetsafe_core_{timestamp}_{pid}.jsonl"
...
def setup_logger(*, level: int | None = None) -> Any:
    ...
    try:
        name = str(Path(name).relative_to(FLEETSAFE_CORE_PROJECT_ROOT))
    ...
    if level is None:
        level_name = os.getenv("FLEETSAFE_CORE_LOG_LEVEL", "INFO")
