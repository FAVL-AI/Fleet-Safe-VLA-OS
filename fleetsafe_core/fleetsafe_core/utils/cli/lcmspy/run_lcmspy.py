...
from fleetsafe_core.utils.cli.lcmspy.lcmspy import GraphLCMSpy, GraphTopic as SpyTopic


def gradient(max_value: float, value: float) -> str:
    """Gradient from cyan (low) to yellow (high) using FleetSafeCore theme colors"""
...
def topic_text(topic_name: str) -> Text:
    """Format topic name with FleetSafeCore theme colors"""
    if "#" in topic_name:
