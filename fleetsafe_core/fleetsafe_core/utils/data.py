...
def _get_user_data_dir() -> Path:
    ...
    if os.environ.get("VIRTUAL_ENV"):
        venv_data_dir = Path(
            f"{os.environ.get('VIRTUAL_ENV')}/lib/python{sys.version_info.major}.{sys.version_info.minor}/site-packages/fleetsafe_core/data"
        )
    ...
    if system == "Linux":
        ...
        if xdg_data_home:
            return Path(xdg_data_home) / "fleetsafe_core"
        return Path.home() / ".local" / "share" / "fleetsafe_core"
    elif system == "Darwin":  # macOS
        return Path.home() / "Library" / "Application Support" / "fleetsafe_core"
    else:
        # Fallback for other systems
        return Path.home() / ".fleetsafe_core"
...
def _get_repo_root() -> Path:
    ...
    if (FLEETSAFE_CORE_PROJECT_ROOT / ".git").exists():
        return FLEETSAFE_CORE_PROJECT_ROOT
    ...
    try:
        ...
        data_dir = Path(tempfile.gettempdir()) / "fleetsafe_core"
        data_dir.mkdir(parents=True, exist_ok=True)
