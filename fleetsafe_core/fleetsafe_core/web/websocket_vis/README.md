...
The `WebsocketVisModule` provides a real-time data for visualization and control of the robot in Foxglove (see `fleetsafe_core/web/command-center-extension/README.md`).
...
```python
from fleetsafe_core.web.websocket_vis.websocket_vis_module import WebsocketVisModule
from fleetsafe_core.core.transport import LCMTransport, pLCMTransport

# Deploy the WebSocket visualization module
websocket_vis = fleetsafe_core.deploy(WebsocketVisModule, port=7779)
...
See `fleetsafe_core/web/command-center-extension/README.md` for how to add the command-center plugin in Foxglove.
