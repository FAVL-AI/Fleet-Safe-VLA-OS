...
fleetsafe_core --replay run drone-basic

# Agentic mode with replay
fleetsafe_core --replay run drone-agentic

# Real drone — indoor (velocity-based odometry)
fleetsafe_core run drone-basic

# Real drone — outdoor (GPS-based odometry)
fleetsafe_core run drone-basic --set outdoor=true

# Agentic with LLM control
fleetsafe_core run drone-agentic
```

To interact with the agent, run `fleetsafe_core humancli` in a separate terminal.
...
### Python (included with FleetSafeCore)
```bash
pip install -e ".[drone]"
```
...
RosettaDrone is an Android app that bridges DJI SDK to MAVLink protocol. Without it, the drone cannot communicate with FleetSafeCore.
...
### Hardware Connection
```
DJI Drone ← Wireless → DJI Controller ← USB → Android Device ← WiFi → FleetSafeCore Computer
```
...
```bash
# Unit tests
pytest -s fleetsafe_core/robot/drone/
...
- Run `fleetsafe_core humancli` to send commands
...
| 7779 | WebSocket | FleetSafeCore web visualization |
| 8765 | WebSocket | Foxglove bridge |
| 7667 | UDP | LCM messaging |

## Coordinate Systems
- **MAVLink/NED**: X=North, Y=East, Z=Down
- **ROS/FleetSafeCore**: X=Forward, Y=Left, Z=Up
