...
from fleetsafe_core.msgs.geometry_msgs import PoseStamped, Quaternion, Vector3
from fleetsafe_core.msgs.sensor_msgs import Image, ImageFormat
from fleetsafe_core.robot.drone.connection_module import DroneConnectionModule
from fleetsafe_core.robot.drone.dji_video_stream import FakeDJIVideoStream

# Drone class removed - use blueprints instead
from fleetsafe_core.robot.drone.mavlink_connection import FakeMavlinkConnection, MavlinkConnection
...
class TestReplayMode(unittest.TestCase):
    ...
    def test_fake_mavlink_connection(self) -> None:
        ...
        with patch("fleetsafe_core.utils.testing.TimedSensorReplay") as mock_replay:
    ...
    def test_connection_module_replay_mode(self) -> None:
        ...
        with patch("fleetsafe_core.robot.drone.mavlink_connection.FakeMavlinkConnection") as mock_fake_conn:
            with patch("fleetsafe_core.robot.drone.dji_video_stream.FakeDJIVideoStream") as mock_fake_video:
...
class TestDroneFullIntegration(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # Mock the FleetSafeCore core module
        self.mock_fleetsafe_core = MagicMock()
        self.mock_fleetsafe_core.deploy.return_value = MagicMock()

        # Mock pubsub.lcm.autoconf
        self.pubsub_patch = patch("fleetsafe_core.protocol.pubsub.lcm.autoconf")
        self.pubsub_patch.start()

        # Mock FoxgloveBridge
        self.foxglove_patch = patch("fleetsafe_core.robot.drone.drone.FoxgloveBridge")
    ...
    @patch("fleetsafe_core.robot.drone.drone.ModuleCoordinator")
    @patch("fleetsafe_core.utils.testing.TimedSensorReplay")
    def test_full_system_with_replay(self, mock_replay, mock_coordinator_class) -> None:
        ...
        mock_coordinator_class.return_value = self.mock_fleetsafe_core
        ...
        self.mock_fleetsafe_core.deploy.side_effect = deploy_side_effect

        # Start the drone system
        drone.start()

        # Verify modules were deployed
        self.assertEqual(self.mock_fleetsafe_core.deploy.call_count, 4)
        ...
        self.mock_fleetsafe_core.stop.assert_called_once()
...
class TestDroneControlCommands(unittest.TestCase):
    ...
    @patch("fleetsafe_core.utils.testing.TimedSensorReplay")
    @patch("fleetsafe_core.utils.data.get_data")
...
class TestVisualServoingEdgeCases(unittest.TestCase):
    ...
    def test_output_clamping(self) -> None:
        ...
        from fleetsafe_core.robot.drone.drone_visual_servoing_controller import (
