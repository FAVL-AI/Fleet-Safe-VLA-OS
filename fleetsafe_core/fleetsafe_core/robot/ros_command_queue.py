...
class ROSCommandQueue:
    ...
    def queue_action_client_request(  # type: ignore[no-untyped-def]
        ...
        **kwargs,
        ...
        logger.info(
            f"Queued action request: {action_name} (ID: {request_id}, Priority: {priority}, Params: {action_params})"
        )
    ...
    def _process_queue(self) -> None:
        ...
        while not self._should_stop:
            ...
            if not self._queue.empty() and self._current_command is None:
                ...
                if self._debug:
                    logger.debug(
                        f"[WebRTC Queue] Status: {self.queue_size} requests waiting | Robot ready: {is_ready} | Robot busy: {is_busy}"
                    )
