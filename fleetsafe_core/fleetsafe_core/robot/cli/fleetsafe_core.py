...
from fleetsafe_core.agents.mcp.mcp_adapter import McpAdapter, McpError
from fleetsafe_core.core.global_config import GlobalConfig, global_config
from fleetsafe_core.core.run_registry import get_most_recent, is_pid_alive, stop_entry
from fleetsafe_core.utils.logging_config import setup_logger
...
def run(
    ...
    logger.info("Starting FleetSafeCore")

    from fleetsafe_core.core.blueprints import autoconnect
    from fleetsafe_core.core.run_registry import (
        LOG_BASE_DIR,
        RunEntry,
    ...
    from fleetsafe_core.robot.get_all_blueprints import get_by_name, get_module_by_name
    from fleetsafe_core.utils.logging_config import set_run_log_dir, setup_exception_handler
    ...
    if conflict:
        typer.echo(
            ...
            f"Run 'fleetsafe_core stop' first.",
            err=True,
        )
        raise typer.Exit(1)
    ...
    # Workers inherit FLEETSAFE_CORE_RUN_LOG_DIR env var via forkserver.
    ...
    if daemon:
        from fleetsafe_core.core.daemon import (
            daemonize,
        ...
        typer.echo("✓ FleetSafeCore running in background\n")
        typer.echo(f"  Run ID:    {run_id}")
        typer.echo(f"  Log:       {log_dir}")
        typer.echo("  Stop:      fleetsafe_core stop")
        typer.echo("  Status:    fleetsafe_core status")
...
@main.command()
def status() -> None:
    """Show the running FleetSafeCore instance."""
    entry = get_most_recent(alive_only=True)
    if not entry:
        typer.echo("No running FleetSafeCore instance")
        return
...
def stop(
    ...
    """Stop the running FleetSafeCore instance."""

    entry = get_most_recent(alive_only=True)
    if not entry:
        typer.echo("No running FleetSafeCore instance", err=True)
        raise typer.Exit(1)
...
def log_cmd(
    ...
    """View logs from a FleetSafeCore run."""
    from fleetsafe_core.core.log_viewer import follow_log, format_line, read_log, resolve_log_path
...
def _get_adapter() -> McpAdapter:
    ...
    from fleetsafe_core.agents.mcp.mcp_adapter import McpAdapter
...
def mcp_list_tools() -> None:
    ...
    try:
        ...
        typer.echo("Error: no running MCP server (is FleetSafeCore running?)", err=True)
        raise typer.Exit(1)
...
def agent_send_cmd(
    ...
    """Send a message to the running FleetSafeCore agent via MCP."""
    try:
...
def restart(
    ...
    """Restart the running FleetSafeCore instance with the same arguments."""
    entry = get_most_recent(alive_only=True)
    if not entry:
        typer.echo("No running FleetSafeCore instance to restart", err=True)
        raise typer.Exit(1)
...
def list_blueprints() -> None:
    ...
    from fleetsafe_core.robot.all_blueprints import all_blueprints
...
def lcmspy(ctx: typer.Context) -> None:
    ...
    from fleetsafe_core.utils.cli.lcmspy.run_lcmspy import main as lcmspy_main
...
def agentspy(ctx: typer.Context) -> None:
    ...
    from fleetsafe_core.utils.cli.agentspy.agentspy import main as agentspy_main
...
def humancli(ctx: typer.Context) -> None:
    ...
    from fleetsafe_core.utils.cli.human.humanclianim import main as humancli_main
...
def top(ctx: typer.Context) -> None:
    ...
    from fleetsafe_core.utils.cli.dtop import main as dtop_main
...
def echo(
    ...
    from fleetsafe_core.robot.cli.topic import topic_echo
...
def send(
    ...
    from fleetsafe_core.robot.cli.topic import topic_send
...
def rerun_bridge_cmd(
    ...
    from fleetsafe_core.visualization.rerun.bridge import run_bridge
