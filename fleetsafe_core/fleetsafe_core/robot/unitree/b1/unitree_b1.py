...
from fleetsafe_core.core.resource import Resource
from fleetsafe_core.core.transport import LCMTransport, ROSTransport
from fleetsafe_core.msgs.geometry_msgs import PoseStamped, TwistStamped
from fleetsafe_core.msgs.nav_msgs.Odometry import Odometry
from fleetsafe_core.msgs.std_msgs import Int32
from fleetsafe_core.msgs.tf2_msgs.TFMessage import TFMessage
from fleetsafe_core.robot.robot import Robot
from fleetsafe_core.robot.unitree.b1.connection import (
    B1ConnectionModule,
    MockB1ConnectionModule,
)
from fleetsafe_core.skills.skills import SkillLibrary
from fleetsafe_core.types.robot_capabilities import RobotCapability
...
class UnitreeB1(Robot, Resource):
    ...
    def __init__(
        ...
        self.joystick = None
        self._fleetsafe_core = ModuleCoordinator(n=2)

        os.makedirs(self.output_dir, exist_ok=True)
        logger.info(f"Robot outputs will be saved to: {self.output_dir}")

    def start(self) -> None:
        """Start the B1 robot - initialize FleetSafeCore, deploy modules, and start them."""

        logger.info("Initializing FleetSafeCore...")
        self._fleetsafe_core.start()

        logger.info("Deploying connection module...")
        if self.test_mode:
            self.connection = self._fleetsafe_core.deploy(MockB1ConnectionModule, self.ip, self.port)  # type: ignore[assignment]
        else:
            self.connection = self._fleetsafe_core.deploy(B1ConnectionModule, self.ip, self.port)  # type: ignore[assignment]
        ...
        if self.enable_joystick:
            from fleetsafe_core.robot.unitree.b1.joystick_module import JoystickModule

            self.joystick = self._fleetsafe_core.deploy(JoystickModule)  # type: ignore[assignment]
        ...
        self._fleetsafe_core.start_all_modules()
